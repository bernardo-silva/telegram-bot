...
def entry_to_string(
    data: datetime,
    requested_data: datetime,
    valor: int,
    parceiro: str,
    prefix: str = "",
) -> str:
    text = f"{prefix}{data.hour:02}h{data.minute:02} {valor}€ {parceiro}"
    if data.day == requested_data.day and data.month == requested_data.month:
        text = f"*__{text}__*"
    return text + "\n"


def mbway(data):
    tabela_premios = pd.read_csv(
        "tabela_premios.csv", index_col="data", parse_dates=["data"]
    )
    tabela_premios = tabela_premios.sort_index()

    premio_diario = tabela_premios.loc[
        (tabela_premios.index.date == data.date())
        & (tabela_premios.tipo.str.contains("Diário"))
    ]

    premio_semanal = (
        tabela_premios.loc[tabela_premios.tipo == "Prémio Semanal"].loc[data:].head(1)
    )
    premio_mensal = (
        tabela_premios.loc[tabela_premios.tipo == "Prémio Mensal"].loc[data:].head(1)
    )

    premio_diario_str = f"Dia {data.day:02}/{data.month:02}:\n"

    for data_indice, premio in premio_diario.iterrows():
        premio_diario_str += entry_to_string(
            data_indice, data, int(premio.valor), premio.parceiro
        )
